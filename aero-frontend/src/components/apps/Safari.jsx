...
const NavSection = ({ width, section, setGoURL }) => {
...
const Safari = ({ width }) => {
  ...
  const pressURL = (e) => {
    const keyCode = e.key;
    if (keyCode === "Enter") setGoURL((e.target).value);
  };

  const buttonColor = state.goURL === "" ? "c-text-400" : "c-text-700";
  const grid = (width) < 640 ? "grid-cols-2" : "grid-cols-3";
  const hideLast = (width) < 640 ? "hidden" : "flex";
